...
class StageTrackReport(models.Model):
    ...
    _name = "signed.call.track.report"
    _auto = False
    ...
    call_signed  = fields.Float('Call signed rate')
    ...
    def _select(self):
        ...
        call_values as ( select id,create_date from mail_message where mail_activity_type_id=2)
            SELECT
                ...
                l.state_id,
                l.stage_id,
                count(m.id)/count(c.id) as call_signed                
    ...
    def _from(self):
        return """
            FROM call_values AS c, mail_tracking_value AS m
        """
    ...
    def _where(self):
        return """
            WHERE
               EXTRACT(YEAR FROM m.create_date)= EXTRACT(YEAR FROM c.create_date)
