...
class account_invoice(models.Model):
    ...
    def calcul_interest(self):
        ...
        for account in account_invoices:
            ...
            print('value total interest---',formatLang(self.env, account.currency_id.round(total_interest) + 0.0, currency_obj=account.currency_id))
            print('account interest ----',account.total_interest)
            if formatLang(self.env, account.currency_id.round(total_interest) + 0.0, currency_obj=account.currency_id) > formatLang(self.env, account.currency_id.round(account.total_interest) + 0.0, currency_obj=account.currency_id):
                print('okkkk')
                vals = {
                    'number_days': diff,
