                        ...
                        try:
                            ...
                            if tx is None or tx.state != 'done':
                                ...
                                if close_subscription:
                                    ...
                                    _logger.debug("Sending Subscription Payment Failure Mail to %s for subscription %s ", subscription.partner_id.email, subscription.id)
                                    ...
                                    if (datetime.datetime.today() - datetime.datetime.strptime(subscription.recurring_next_date, '%Y-%m-%d')).days in [0, 3, 7, 14]:
                                        template = template_res.browse(template_id)
                                        template.with_context(email_context).send_mail(subscription.id)
                                        _logger.debug("Sending Payment Failure Mail to %s for subscription %s", subscription.partner_id.email, subscription.id)
